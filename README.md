...
- **디스코드 챗봇 생성**:
    ...
    - **토큰 발급**: 챗봇과 서버 간의 통신을 위해 **봇 토큰**을 발급받고, 이를 코드에 적용하였습니다.
    - **봇 아이디 설정**: 생성된 챗봇을 디스코드 서버에 초대하고, 해당 **봇 아이디**를 설정하여 프로젝트에 연동했습니다.

- **알림 시스템 구현**:
   - **임계값 설정**:
        - 임계값은 **측정 장소의 특성**을 고려하여 **평균적인 값을 경험적으로 분석**한 뒤, 실험을 통해 임의로 설정하였습니다.
   ...
   - **임계값 미만 시 정상 알림**:
        - PM2.5 값이 임계값 이하일 경우, "정상 수준입니다"라는 메시지가 전송됩니다.
    ...
    - **핸드폰 알림 연동**:
        - **디스코드 모바일 앱**을 통해 사용자 핸드폰에도 실시간 알림이 전달됩니다.
