...
- **(2) 실내외 데이터의 상시 모니터링**
    - Grove 센서와 에어코리아 API 데이터를 지속적으로 수집하여 장기적인 패턴 분석이 가능하도록 데이터베이스를 구축해야 합니다.  
    - 실시간 모니터링뿐만 아니라 과거 데이터 시각화 및 예측 기능을 강화하면 사용자에게 더 유용한 정보를 제공할 수 있습니다.

- **(3) 디스코드 외 알림 채널 확장**
    - 디스코드 외에 SMS, Slack, 이메일 등의 알림 채널을 추가하여 사용자 접근성을 높입니다.  
    - 모바일 애플리케이션과 연동해 푸시 알림 기능을 구현하면 더욱 직관적인 사용자 경험을 제공할 수 있습니다.

- **(4) 센서 데이터 정확도 향상**
    - 현재 Grove 센서의 측정 데이터는 특정 환경 조건에 따라 오차가 발생할 수 있습니다.  
    - **고성능 센서**를 추가 도입하거나 여러 센서 데이터를 결합하는 센서 퓨전 기법을 통해 정확도를 개선할 수 있습니다.
...
- **(7) Gradio UI 고도화**
    - 현재 Gradio 인터페이스를 개선해 사용자 맞춤형 대시보드를 제공할 수 있습니다.  
    - 차트 시각화, 지역별 비교 데이터, 날씨 및 대기질 예측 등의 기능을 추가하여 사용자 경험을 향상시킵니다.
...
이번 산업인공지능 젯슨 나노 수업을 통해서 가장 와닿게 느꼈던 점은 실내 공기청정기의 원리를 알 수 있었다는 점 이었습니다. 내가 직접 미세먼지농도를 측정할 수 있고 간단하지만 공기청정기를 만든 것 같아 의미있던 시간이었던 것 같습니다. 또한 자동으로 저의 핸드폰에 알림이 가는 기능까지 만들 수 있어서 정말 개발자가 된 느낌이었고, 이 경험을 살려 미래에 대기분야에서 직업을 가질 수 있는 기회가 되면 좋겠습니다.
